//------------------------------------------------------------------------------
// <auto-generated>
//     這個程式碼是由範本產生。
//
//     對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//     如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace WebStoreHouse.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class E_StoreHouseStock
    {
        public int serialno { get; set; }
        public Nullable<int> sno { get; set; }
        public string wono { get; set; }
        public string cust_wono { get; set; }
        public string eng_sr { get; set; }
        public Nullable<int> order_count { get; set; }
        public Nullable<int> quantity { get; set; }
        public Nullable<int> kf10 { get; set; }
        public Nullable<int> kq30 { get; set; }
        public string sap_in { get; set; }
        public string position { get; set; }
        public Nullable<int> acc_in { get; set; }
        public Nullable<int> outed { get; set; }
        public Nullable<int> notout { get; set; }
        public Nullable<int> borrow { get; set; }
        public Nullable<System.DateTime> due_date { get; set; }
        public string mark { get; set; }
        public string inputdate { get; set; }
        public string package { get; set; }
        public string output_local { get; set; }
        public string Igroup { get; set; }
        public string del_flag { get; set; }
        public string nowono { get; set; }
        public Nullable<int> box_quantity { get; set; }
        public string transportation { get; set; }
    }
}
